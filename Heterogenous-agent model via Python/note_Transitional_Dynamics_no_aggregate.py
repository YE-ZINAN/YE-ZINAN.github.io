...
def steady_state_policy(N,K,alpha,Pi,a_space,w,b,r,tau,eta,beta,tol):
    ...
    for _ in range(10000): #Maximum steps of iteration: 10000
        ...
        if _>0 and np.max(np.abs(a - a_old)) < 1E-15:
            if Hs==0:
...
for _ in range(10000):
    w=wfn(N,K_ss_1,alpha)
    r=rfn(N,K_ss_1,alpha,delta)
    Va_ss, a_ss ,c_ss = steady_state_policy(N,K_ss_1,alpha,Pi,a_space,w,b,r,tau,eta,beta,tol)
